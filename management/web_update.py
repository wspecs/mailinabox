...
def get_web_domains(env, include_www_redirects=True, exclude_dns_elsewhere=True):
  ...
  domains |= set(get_domains_with_a_records(env, True))
...
def get_domains_with_a_records(env, points_to_current_server=False):
  domains = set()
  ...
  for domain, rtype, value in dns:
    if rtype == "CNAME":
      domains.add(domain)
    if rtype in ("A", "AAAA"):
      same_server = value in ("local", env['PUBLIC_IP'])
      if same_server == points_to_current_server:
        domains.add(domain)
  return domains
