...
# The systemd-resolved service provides local DNS name resolution. By default it
# is a recursive stub nameserver, which means it simply relays requests to an
# external nameserver, usually provide by your ISP or configured in /etc/systemd/resolved.conf.
# However because we have higher security goals, we can't trust the connection
# to the external nameserver. We will also configure postfix to use DANE, which
# uses DNSSEC to find TLS certificates for remote servers. The only way to trust
# that DNSSEC validation is performed is to perform it locally. So we turn on
# systemd-resolved's setting to perform local DNSSEC validation on all requests.
# See the man page for resolved.conf.
tools/editconf.py /etc/systemd/resolved.conf DNSSEC=yes
